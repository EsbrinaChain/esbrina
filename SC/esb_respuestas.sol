...
contract responder is preguntar{
    ...
    function creaRespuesta(uint id_preg, string memory texto,
                           string memory nombre, string memory apellidos, 
                           string memory alia) public {
      require(total_preg > 0, "No existen preguntas en en el sistema.");
      require(preguntas[id_preg].creada > 0, "No existe una pregunta con Fecha de creacion 0.");
      require(preguntas[id_preg].estado == estado_preg.abierta,
      ...
      if(block.timestamp - preguntas[id_preg].creada >= tiempo_respuesta){
         ...
         emit RespuestaFueraDeTiempo(id_preg, (block.timestamp - preguntas[id_preg].creada) % 86400, preguntas[id_preg].autor);
         emit InicioVotacion(id_preg);
      } else if(num_resp_act >= cupo_respuestas){
        preguntas[id_preg].estado = estado_preg.votando;
        preguntas[id_preg].fecha_votacion = block.timestamp;
        emit RespuestaFueraDeCupo(id_preg, cupo_respuestas, preguntas[id_preg].autor);
        emit InicioVotacion(id_preg);
      } else{
        ...
        require(preguntas[id_preg].autor != msg.sender, "El usuario autor no puede responder a sus preguntas.");
        // El usuario solo puede responder 1 vez a una pregunta
        require(!calcAdrRespuestasAPregunta(id_preg, msg.sender), "No se puede responder mas de 1 vez.");
        // Si no es usuario tenemos que recibir un nombre o un apellido o un alias
        if (!usuarios[msg.sender].existe){
            require(keccak256(abi.encodePacked(nombre)) != keccak256(abi.encodePacked("")) 
                 || keccak256(abi.encodePacked(apellidos)) != keccak256(abi.encodePacked(""))
                 || keccak256(abi.encodePacked(alia)) != keccak256(abi.encodePacked("")), 
        ...
        total_resp++;
        pregNumResp[id_preg]++;
        preg_resp[id_preg][pregNumResp[id_preg]]= Respuesta(pregNumResp[id_preg], id_preg, 0,msg.sender, texto, false);
        emit RespuestaCreada(id_preg,pregNumResp[id_preg], msg.sender);
      }
    }
    function vencePregunta(uint idx_preg) public view returns(string memory){
        require(preguntas[idx_preg].estado != estado_preg.anulada,"La pregunta esta anulada.");
        uint[] memory rsp = calcRespAPreg(idx_preg);
        string memory s = "";

        for (uint i=1; i < rsp.length+1; i++){
            if(preg_resp[idx_preg][i].ganadora){
                s = string(abi.encodePacked(usuarios[preg_resp[idx_preg][i].autor].app,", ",
                usuarios[preg_resp[idx_preg][i].autor].nom," (",
                usuarios[preg_resp[idx_preg][i].autor].aliase,
                "); ",s));
            }
        } 
        return s;
    ...
    function adrVencePregunta(uint idx_preg) internal view returns(address[] memory){
        require(preguntas[idx_preg].estado != estado_preg.anulada,"La pregunta esta anulada.");
        uint[] memory rsp = calcRespAPreg(idx_preg);
        uint j=0;
        for (uint i=1; i < rsp.length+1; i++){
        ...
        address[] memory adrGanadores = new address[](j);
        j=0;
        for (uint i=1; i < rsp.length+1; i++){
            if(preg_resp[idx_preg][i].ganadora){
               adrGanadores[j] =  preg_resp[idx_preg][i].autor;
               j++;
            }
        }
        return adrGanadores;
    }
    function adrRespondeVence(uint idx_preg, address _adr) public view returns(bool, bool){
        if(preguntas[idx_preg].estado == estado_preg.consulta){
            uint[] memory rsp = calcRespAPreg(idx_preg);
            for (uint i=1; i < rsp.length+1; i++){
                if(preg_resp[idx_preg][i].ganadora && preg_resp[idx_preg][i].autor == _adr){
                    return (true, true);
                }
                if(!preg_resp[idx_preg][i].ganadora && preg_resp[idx_preg][i].autor == _adr){
                    return (true, false);
    ...
    // Permite conocer el precio que se debe enviar (no autores) para consultar la respuesta a una pregunta.
    function consultaPrecioRespuestaVotada(uint idx_preg) public view returns(uint){
        return uint(preguntas[idx_preg].recompensa /2);
    }
