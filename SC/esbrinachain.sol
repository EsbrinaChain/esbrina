...
contract esbrinachain is responder {
    ...
    function valoresVariables() public view returns(uint, uint, uint, uint, uint, uint){
        return (tiempo_respuesta, tiempo_votacion, cupo_respuestas, total_preg, total_resp, num_incr_recompensa);
    }

    function votarRespuesta(uint idx_preg, uint idx_resp) public {
        if((block.timestamp - preguntas[idx_preg].fecha_votacion) >= tiempo_votacion){
            require(preguntas[idx_preg].estado == estado_preg.votando, "La pregunta no se puede VOTAR.");
            ...
            if (preguntas[idx_preg].votada){
                ...
                emit PreguntaAnulada(idx_preg, "Pregunta anulada por falta de votaciones");
            }
      }else{
        ...
        require(preguntas[idx_preg].autor != msg.sender, "El autor no puede VOTAR sus preguntas.");
        // como solo puede votar 1 vez no debe existir un elemento dado una address y un id_preg
        require(votaciones[msg.sender][idx_preg] == 0, "Un usuario solo puede votar 1 vez una pregunta.");
        uint[] memory rsp = calcRespAPreg(idx_preg);
        require(preguntas[idx_preg].creada != 0,"No existe una pregunta con este ID.");
        require(idx_resp > 0 && idx_resp <= rsp[rsp.length-1],"No existe ese ID de respuesta para esa pregunta.");
    ...
    function resolucionVotacion(uint idx_preg) internal {
        ...
        if (rsp.length == 0) {
            preguntas[idx_preg].estado = estado_preg.anulada;
                ...
                uint j=0; resp_mas_votada[j] = 1;
                for(uint i=2; i <= rsp[rsp.length-1]; i++){
                    if(preg_resp[idx_preg][i].votos > preg_resp[idx_preg][resp_mas_votada[j]].votos){
                        resp_mas_votada[j] = i;
                    }else{
                       if(preg_resp[idx_preg][i].votos == preg_resp[idx_preg][resp_mas_votada[j]].votos){
                            j++; resp_mas_votada[j] = i;
                       }
                    }
                }
            j=0;
    ...
    function iniciaVotacion(uint idx_preg) public soloAdmin {
        require(preguntas[idx_preg].estado == estado_preg.abierta, "La pregunta no esta abierta.");
        require(calcRespAPreg(idx_preg).length > 0,"No se han dado respuestas a esta pregunta.");
    ...
    function finalizaVotacion(uint idx_preg) public soloAdmin {
        require(preguntas[idx_preg].estado == estado_preg.votando, "La pregunta no se esta votando.");
        preguntas[idx_preg].estado = estado_preg.consulta;
    ...
    function admCfgTiempoRespuesta(uint num, uint t, string memory units) external soloAdmin{
        ...
        uint valor;
        if (keccak256(abi.encodePacked(string(units))) == keccak256(abi.encodePacked("m"))) valor = t*60;
        if (keccak256(abi.encodePacked(string(units))) == keccak256(abi.encodePacked("h"))) valor = t*3600;
        if (keccak256(abi.encodePacked(string(units))) == keccak256(abi.encodePacked("d"))) valor = t*86400;
        
        if (num == 1) tiempo_respuesta = valor;
        else if(num == 2) tiempo_votacion = valor;
        else { if(num == 3) cupo_respuestas = t; } 
    }
    ...
    function calcPrestigio(address _usr) internal returns(int){
        require(preguntas[1].creada!=0,"No existen preguntas en el sistema.");
        require(preg_resp[1][1].autor!=address(0),"No existen respuestas en el sistema.");
        int venceUsr=0; int noVenceUsr=0;
        int prestigio=0;
        for(uint i=1; i <= total_preg; i++){
            if (preguntas[i].estado==estado_preg.consulta && calcAdrRespuestasAPregunta(i, _usr)) {
    ...
    function consUsrPrestigio(address _usr) external payable returns(int){
        require(preguntas[1].creada!=0,"No existen preguntas en el sistema.");
        require(preg_resp[1][1].autor!=address(0),"No existen respuestas en el sistema.");
        require(msg.value >= 0.0005 ether, unicode"Para conocer su prestigio debe enviar 0.0005 ETH.");
        int venceUsr=0; int noVenceUsr=0;
        int prestigio=0;
        for(uint i=1; i <= total_preg; i++){
            if (preguntas[i].estado==estado_preg.consulta && calcAdrRespuestasAPregunta(i, _usr)) {
