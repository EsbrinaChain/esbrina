...
contract esbAdmin {
    ...
    modifier soloAdmin() {
        require(
            msg.sender == _admin,
            unicode"Debe ser ADMIN para realizar esta operación."
        );
        _;
    }
}
...
contract preguntar is esbAdmin {
    ...
    function creaPregunta(
        ...
        require(
            msg.value > 0,
            "No se ha enviado recompensa para resolver la pregunta."
        );
        if (!usuarios[msg.sender].existe) {
            require(
                bytes(nombre).length > 0 ||
                    bytes(apellidos).length > 0 ||
                    bytes(alia).length > 0,
                " No se ha facilitado ni nombre, ni apellidos, ni alias."
            );
            total_usuarios++;
    ...
    function subirRecompensaPreg(uint idx_preg) public payable {
        ...
        require(
            ...
            "La pregunta no esta abierta a respuestas."
        );
        if (msg.value > 0) {
    ...
    function estadoPreg(uint idx_preg) public view returns (string memory) {
        ...
        if (preguntas[idx_preg].estado == estado_preg.abierta)
            estado = "Abierta.";
        if (preguntas[idx_preg].estado == estado_preg.votando)
            estado = "Votando.";
        if (preguntas[idx_preg].estado == estado_preg.consulta)
            estado = "Consulta.";
        if (preguntas[idx_preg].estado == estado_preg.anulada)
            estado = "Anulada.";
